...
export const scrapeOrchestrator = async (startDate: string, endDate: string) => {
  ...
  if (cacheSchedules === 'No Cache' || cacheCalendar === 'No Cache') {
    ...
    if (cacheSchedules === 'No Cache') {
      saveCacheToRedis(currSchedules, RedisNamespace.WINGS_SCHEDULES);
    }
    // 캘린더 캐시데이터 등록
    if (cacheCalendar === 'No Cache') {
      saveCacheToRedis(updatedCalendars, RedisNamespace.GOOGLE_CALENDAR_EVENTS);
    }
    return '초기화 설정 완료';
  }
  ...
  // ! DEBUG
  console.log(newEventNumbers.size, diffEventNumbers.size, diffEventFieldValue);
